    ...
    <main className="mx-auto flex min-h-screen w-full max-w-7xl flex-col gap-6 px-6 py-8">
      ...
      {result.digital_twin ? (
        <section className="rounded-[2rem] border border-[color:var(--color-line)] bg-white p-6 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-slate-500">Digital twin layer</p>
          <div className="mt-2 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h2 className="text-2xl font-semibold text-slate-950">{result.digital_twin.primary_framework}</h2>
              <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-600">{result.digital_twin.update_mode}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <StatusPill>{result.digital_twin.twin_state}</StatusPill>
              <StatusPill>patient-state tracking</StatusPill>
            </div>
          </div>

          <div className="mt-5 grid gap-3 lg:grid-cols-3">
            {result.digital_twin.layers.map((layer) => (
              <article
                key={layer.id}
              ...
              >
                ...
                <p className="mt-3 text-sm leading-7 text-slate-600">{layer.role}</p>
                <p className="mt-3 text-sm leading-7 text-slate-600">{layer.note}</p>
                <div className="mt-3 flex flex-wrap gap-2">
                  {layer.sources.map((source) => (
                    <StatusPill key={`${layer.id}-${source}`}>{source}</StatusPill>
                  ))}
                </div>
              </article>
            ))}
